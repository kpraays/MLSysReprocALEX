...
#define PAYLOAD_TYPE long double
/*
 * Required flags:
...
int main(int argc, char* argv[]) {
  ...
  std::sort(values, values + init_num_keys, [](const std::pair<KEY_TYPE, PAYLOAD_TYPE>& a, const std::pair<KEY_TYPE, PAYLOAD_TYPE>& b) { return a.first < b.first; });



  myBTree.bulk_load(values, values+init_num_keys);
  
  double bulkload_elapsed_time =
    std::chrono::duration_cast<std::chrono::nanoseconds>(
        std::chrono::high_resolution_clock::now() - bulkload_start_time)
        .count();
  std::cout << "Bulk load time for " << init_num_keys << " keys is:" << bulkload_elapsed_time << std::endl;
  ...
  while (true) {
    ...
    if (i > 0) {
      ...
      for (int j = 0; j < num_lookups_per_batch; j++) {
        ...
        // Use stx-btree's find method to get an iterator to the key-value pair
        auto it = myBTree.find(key);

        // Check if the key is found
        if (it != myBTree.end()) {
            // If found, accumulate the payload value to the sum
            sum += it->second;
        }

        // PAYLOAD_TYPE* payload = myBTree.get_payload(key);
        // if (payload) {
        //   sum += *payload;
        // }
      }
    ...
    for (; i < num_keys_after_batch; i++) {
      myBTree.insert(keys[i], static_cast<PAYLOAD_TYPE>(gen_payload()));
    }
